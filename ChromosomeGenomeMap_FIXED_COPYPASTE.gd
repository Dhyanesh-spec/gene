...
func _ready() -> void:
	...
	# The child becomes wide, causing the parent ScrollContainer to scroll
	# horizontally. Height stays fixed: no vertical scroll is necessary.
...
func add_gene(
	...
	if not gene.has("chromosome"):
		push_warning(
            "Gene is missing chromosome."
		)
		return

	if not gene.has("start"):
		push_warning(
            "Gene is missing genomic start."
		)
		return
...
## Replacements use a thicker orange block.
func replace_gene(
	gene: Dictionary
) -> void:
